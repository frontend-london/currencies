...
import { render, Simulate, fireEvent, cleanup, waitForElement, waitForDomChange, wait } from 'react-testing-library'
import Header from './header';
import currencies from '../../../public/currencies.test.json';

beforeEach(function () {
  global.handleCurrentDateChange = jest.fn(async () => { });
});
...
test('Current date select has correct options and is set correctly ', async () => {
...
test('Current date change handler triggered ', async () => {
  const { getBySelectText } = renderComponent()
  const select = getBySelectText(currentDate)
